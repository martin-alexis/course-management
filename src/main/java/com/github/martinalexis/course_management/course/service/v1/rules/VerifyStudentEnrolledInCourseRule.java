...
@Service
public class VerifyStudentEnrolledInCourseRule {

    public void execute(UserModel user, CourseModel course) {
        boolean isStudent = course.getUserCourses().stream()
                .anyMatch(uc -> uc.getUser().getIdUser().equals(user.getIdUser()) && uc.getRole().getRole().equals(RoleEnum.STUDENT));

        if (!isStudent) {
            throw new InvalidCourseRoleException(RoleEnum.STUDENT);
        }
    }
}
